...
frappe.ui.form.on("FT Stock RM List", {
    ...
    name1(frm) { update_computed_name(frm); },
    stock_rm_type(frm) { update_computed_name(frm); },
    thickness_mm(frm) { update_computed_name(frm); },
    kg__sqm(frm) { update_computed_name(frm); },
    kg__meter(frm) { update_computed_name(frm); },
    grade(frm) { update_computed_name(frm); },
    thk(frm) { update_computed_name(frm); }
});
...
function set_grade_filter(frm) {
    frm.set_query("grade", function () {
        if (frm.doc.section_type === "Section") {
            return { filters: { bis_section: ["!=", ""] } };
        }
        if (frm.doc.section_type === "Plate") {
            return { filters: { bis_plate: ["!=", ""] } };
        }
    });
}
...
function update_computed_name(frm) {
    ...
    if (frm.doc.section_type === "Plate") {
        ...
        if (frm.doc.thickness_mm !== undefined && frm.doc.thickness_mm !== null) {
            ...
            if (thickness.includes(".")) {
                let parts = thickness.split(".");
                thickness = parts[0].padStart(2, "0") + "." + parts[1];
            } else {
                thickness = thickness.padStart(2, "0");
            }
            size_part = thickness + (thk_text ? " " + thk_text : "");
    ...
    if (!frm.doc.grade) {
        frm.set_value("computed_name", [base_name, size_part].filter(Boolean).join(" "));
        return;
    }

    frappe.call({
        method: "frappe.client.get",
        args: { doctype: "FT Material Grade Catalogues", name: frm.doc.grade },
        callback: function (res) {
            if (res.message) {
                let g = res.message;
                grade_display = g.grade || frm.doc.grade;
                main_bis = g.bis || "";
                type_bis = frm.doc.section_type === "Section"
                    ? (g.bis_section || "")
                    : (g.bis_plate || "");

                let final_name = [base_name, size_part, type_bis, main_bis, grade_display]
                    .filter(Boolean)
                    .join(" ");
