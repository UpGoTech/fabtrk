...
#             dp.single_weight,
#             dp.total_weight,
#             COALESCE(dp.quantity, 0) * COALESCE(pod.required_qty, 0) AS po_required_qty,
#             COALESCE(dp.quantity, 0) * COALESCE(pod.required_qty, 0) * COALESCE(dp.single_weight, 0) AS po_item_total_weight
#         FROM `tabFT Drawing Parts` dp
#         LEFT JOIN `tabFT Add Drawing` ad ON ad.name = dp.drawing_number
#         LEFT JOIN `tabFT Project` p ON p.name = ad.project_number
...
#             round(float(d.get("po_item_total_weight") or 0.0), 3)
#         )
...
# ---------------------- Snapshot Save -isme drawing number fled add huyi hai pr value nhi ayi-------------
# @frappe.whitelist()
# def save_row_data(sr_no, project, item_name, item_count, quantity, lenght, width, total_weight):
#     import json
#     from frappe.utils import now_datetime
...
# ///////jo exesting hai usi me raname kra hai is se solve krna hia ///////
@frappe.whitelist()
def save_row_data(sr_no, project, item_name, item_count, quantity, lenght, width, total_weight):
    import json
    from frappe.utils import now_datetime
