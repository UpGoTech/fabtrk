					...
					callback: function (r) {
						...
						if (total_row) {
                            ...
                            <tr style="background:#f0f4ff; border-top:2px solid #1F4E79; box-shadow:0 -2px 5px rgba(0,0,0,0.07);">
                                ...
                                <td style="text-align:center; font-weight:700; font-size:14px;">${peq}</td>
						...
						// ── colgroup — 17 columns (single_unit_surface_area removed) ──
						let colgroup = `
                        <colgroup>
...
function attach_sticky_total_footer(report) {
	...
	data.forEach(d => {
		...
		totals.single_unit_surface_area += parseFloat(d.single_unit_surface_area || 0);
		totals.total_surface_area += parseFloat(d.total_surface_area || 0);
	});
	...
	columns.forEach(function (col, i) {
		...
		if (fn === "item_name") {
			...
			let v = totals[fn];
			if (["lenght", "width", "total_weight", "po_total_weight", "single_unit_surface_area", "total_surface_area"].includes(fn)) {
